...
import { Loader2, CheckCircle, Trophy, ArrowRight, Gift } from "lucide-react";
...
export default function OnboardingPage() {
  ...
  async function submitProfile() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.error("Failed to update profile", error.error || "Please try again");
      }
    } catch (error) {
  ...
  useEffect(() => {
    if (user && formData.name && formData.username && formData.country && !celebrationData && !loading) {
      submitProfile();
    }
  }, [user]);
      ...
      <div className="w-full max-w-7xl bg-white rounded-3xl shadow-2xl overflow-hidden flex flex-col lg:flex-row min-h-[800px]">
        ...
        <div className="max-w-md w-full space-y-8">
          <div className="text-center lg:text-left">
            <h2 className="text-3xl font-bold text-slate-900">Complete Profile</h2>
            <p className="text-slate-500 mt-2">Almost there! Complete your profile to earn 150 points instantly.</p>
          </div>
